							   ...
							   const homekit_value_t *value)
{
	...
	if (ch->permissions & homekit_permissions_paired_read)
	{
		homekit_value_t v = value ? *value : ch->getter_ex ? ch->getter_ex(ch)
														   : ch->value;
...
void homekit_server_on_pair_setup(client_context_t *context, const byte *data, size_t size)
{
	...
	case 3:
	{
		...
		free(server_proof);
		send_tlv_response(context, response);
		CLIENT_DEBUG(context, "response sent");
	...
	CLIENT_DEBUG(context, "Server on pair setup end");
	tlv_free(message);
...
void homekit_server_on_pair_verify(client_context_t *context, const byte *data, size_t size)
{
	...
	case 3:
	{
		...
		size_t device_info_size = context->verify_context->device_public_key_size + context->verify_context->accessory_public_key_size + tlv_device_id->size;
#ifdef PAIRVERIFY_USE_MEMORYBUFFER
		...
		for (homekit_service_t **service_it = accessory->services; *service_it; service_it++)
		{
			...
			for (homekit_characteristic_t **ch_it = service->characteristics; *ch_it; ch_it++)
			{
				...
				write_characteristic_json(json, context, ch,
										  (characteristic_format_t)(characteristic_format_type | characteristic_format_meta | characteristic_format_perms | characteristic_format_events),
										  NULL);
				json_object_end(json);
			}
...
void homekit_server_on_get_characteristics(client_context_t *context)
{
	...
	else
	{
		...
		CLIENT_INFO(context, "Error sending 207 response");
		DEBUG_HEAP();
	}
...
HAPStatus process_characteristics_update(const cJSON *j_ch, client_context_t *context)
{
	...
	if (!ch)
	{
		CLIENT_ERROR(context,
					 "Failed to process request to update %d.%d: "
	...
	if (j_value)
	{
		...
		case homekit_format_int:
		{
			...
			if (ch->valid_values_ranges.count)
			{
				...
				for (int i = 0; i < ch->valid_values_ranges.count; i++)
				{
					if (value >= ch->valid_values_ranges.ranges[i].start && value <= ch->valid_values_ranges.ranges[i].end)
					{
						matches = true;
						break;
					}
		...
		case homekit_format_float:
		{
			...
			if ((ch->min_value && value < *ch->min_value) || (ch->max_value && value > *ch->max_value))
			{
	...
	if (j_events)
	{
		...
		{
			CLIENT_ERROR(context,
						 "Failed to set notification state for %d.%d: "
											  ...
											  size_t size)
{
	...
	HAPStatus *statuses = (HAPStatus *)malloc(
		sizeof(HAPStatus) * esphap_cJSON_GetArraySize(characteristics));
	...
	else
	{
		...
		for (int i = 0; i < esphap_cJSON_GetArraySize(characteristics); i++)
		{
			...
			//json_uint32(json1, esphap_cJSON_GetObjectItem(j_ch, "aid")->valueint);
			json_integer(json1, esphap_cJSON_GetObjectItem(j_ch, "aid")->valueint);
			json_string(json1, "iid");
			//json_uint32(json1, esphap_cJSON_GetObjectItem(j_ch, "iid")->valueint);
...
void homekit_server_on_pairings(client_context_t *context, const byte *data, size_t size)
{
	...
	case TLVMethod_AddPairing:
	{
		...
		if (pairing)
		{
			...
			if (pairing_public_key_size != tlv_device_public_key->size || memcmp(tlv_device_public_key->value, pairing_public_key,
																				 pairing_public_key_size))
			{
				CLIENT_ERROR(context,
...
int homekit_server_on_url(http_parser *parser, const char *data, size_t length)
{
	...
	if (parser->method == HTTP_GET)
	{
		...
		else
		{
			...
			if (length >= url_len && !strncmp(data, url, url_len) && (data[url_len] == 0 || data[url_len] == '?'))
			{
...
http_parser_settings make_http_parser_settings()
{
	...
	settings.on_message_begin = homekit_server_on_message_begin;
...
void homekit_client_process(client_context_t *context)
{
	...
	//CLIENT_DEBUG(context, "homekit_client_process");
	...
	do
	{
		...
		if (available_len > 0)
		{
			...
			// read or readBytes
			CLIENT_DEBUG(context, "Reading %d bytes", size);
			data_len = context->socket->read(context->server->data + data_available, size);
		}
		//CLIENT_DEBUG(context, "Read data_len:%d,available_len:%d,data_available:%d", data_len, available_len, data_available);
		if (data_len == 0)
		{
		...
		if (context->encrypted)
		{
			...
			//client_decrypt_(context, context->server->data, data_len, NULL, &decrypted_size);

			int r = client_decrypt_(context, context->server->data, data_len, context->server->data + 2, &decrypted_size);
	...
	if (!http_should_keep_alive(&context->server->parser))
	{ //fix #64 , after connection close , parser is dead
...
client_context_t *homekit_server_accept_client(homekit_server_t *server)
{
	...
	CLIENT_INFO(context, "new client accepted");
	return context;
}
...
bool homekit_client_need_process_data(client_context_t *context)
{
	if (context)
	{
		return (context->step >= HOMEKIT_CLIENT_STEP_PAIR_SETUP_1OF3 && context->step < HOMEKIT_CLIENT_STEP_PAIR_VERIFY_2OF2);
	}
	return false;
}
...
void homekit_server_init(homekit_server_config_t *config)
{
	...
	if (config->password)
	{
		...
		if (strlen(p) != 10 || !(ISDIGIT(p[0]) && ISDIGIT(p[1]) && ISDIGIT(p[2]) && p[3] == '-' && ISDIGIT(p[4]) && ISDIGIT(p[5]) && p[6] == '-' && ISDIGIT(p[7]) && ISDIGIT(p[8]) && ISDIGIT(p[9])))
		{
			ERROR("Error initializing HomeKit accessory server: "
	...
	if (config->setupId)
	{
		...
		if (strlen(p) != 4 || !(ISBASE36(p[0]) && ISBASE36(p[1]) && ISBASE36(p[2]) && ISBASE36(p[3])))
		{
			ERROR("Error initializing HomeKit accessory server: "
