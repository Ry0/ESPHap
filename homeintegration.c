...
homekit_service_t *hap_new_lightbulb_service(const char *szname, hap_callback cb, void *context)
{
	...
	return NEW_HOMEKIT_SERVICE(LIGHTBULB, .primary = true, .characteristics = (homekit_characteristic_t *[]){NEW_HOMEKIT_CHARACTERISTIC(NAME, szname), NEW_HOMEKIT_CHARACTERISTIC(ON, true, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context), ), NULL});
}
homekit_service_t *hap_new_lightbulb_dim_service(const char *szname, hap_callback cb, void *context)
{

	return NEW_HOMEKIT_SERVICE(LIGHTBULB, .primary = true, .characteristics = (homekit_characteristic_t *[]){NEW_HOMEKIT_CHARACTERISTIC(NAME, szname), NEW_HOMEKIT_CHARACTERISTIC(ON, true, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context), ), NEW_HOMEKIT_CHARACTERISTIC(BRIGHTNESS, 100, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)), NULL});
}
...
{
	return NEW_HOMEKIT_SERVICE(LIGHTBULB, .primary = true, .characteristics = (homekit_characteristic_t *[]){NEW_HOMEKIT_CHARACTERISTIC(NAME, szname), NEW_HOMEKIT_CHARACTERISTIC(ON, true, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)),

																											 NEW_HOMEKIT_CHARACTERISTIC(BRIGHTNESS, 100, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)),

																											 NEW_HOMEKIT_CHARACTERISTIC(HUE, 0, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)), NEW_HOMEKIT_CHARACTERISTIC(SATURATION, 0,
...
homekit_service_t *hap_new_rgbstrip_service_ex(const char *szname, hap_callback cb, void *context)
{
	...
	return NEW_HOMEKIT_SERVICE(TELEVISION, .primary = true, .characteristics = (homekit_characteristic_t *[]){NEW_HOMEKIT_CHARACTERISTIC(NAME, szname), NEW_HOMEKIT_CHARACTERISTIC(ACTIVE_IDENTIFIER, 0, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)), NEW_HOMEKIT_CHARACTERISTIC(ACTIVE, true, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)), NEW_HOMEKIT_CHARACTERISTIC(CONFIGURED_NAME, szname, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)), NEW_HOMEKIT_CHARACTERISTIC(SLEEP_DISCOVERY_MODE, 0,

																																																																																																																																				   .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)),
																											  NEW_HOMEKIT_CHARACTERISTIC(PICTURE_MODE, 0,
...
homekit_service_t *hap_add_temp_hum_as_accessory(int acctype, const char *szname, homekit_service_t **pp_temp, homekit_service_t **pp_hum)
{
	...
	homekit_service_t *baseservice = hap_new_homekit_accessory_service(szname, "0");
	homekit_service_t *temp = NEW_HOMEKIT_SERVICE(TEMPERATURE_SENSOR, .primary = true, .characteristics = (homekit_characteristic_t *[]){NEW_HOMEKIT_CHARACTERISTIC(NAME, szname), NEW_HOMEKIT_CHARACTERISTIC(CURRENT_TEMPERATURE, 0), NULL});
...
homekit_service_t *hap_add_hum_as_accessory(int acctype, const char *szname)
{
	...
	homekit_service_t *hum = NEW_HOMEKIT_SERVICE(HUMIDITY_SENSOR, .primary = true, .characteristics = (homekit_characteristic_t *[]){NEW_HOMEKIT_CHARACTERISTIC(NAME, szname), NEW_HOMEKIT_CHARACTERISTIC(CURRENT_RELATIVE_HUMIDITY, 0), NULL});
	homekit_service_t *svc[3];
	svc[0] = baseservice;
	svc[1] = hum;
	svc[2] = NULL;
...
{
	return NEW_HOMEKIT_SERVICE(GARAGE_DOOR_OPENER, .primary = true, .characteristics = (homekit_characteristic_t *[]){NEW_HOMEKIT_CHARACTERISTIC(NAME, szname), NEW_HOMEKIT_CHARACTERISTIC(CURRENT_DOOR_STATE, HOMEKIT_CHARACTERISTIC_CURRENT_DOOR_STATE_CLOSED, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)), NEW_HOMEKIT_CHARACTERISTIC(TARGET_DOOR_STATE, HOMEKIT_CHARACTERISTIC_TARGET_DOOR_STATE_CLOSED, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)), NEW_HOMEKIT_CHARACTERISTIC(OBSTRUCTION_DETECTED, HOMEKIT_CHARACTERISTIC_TARGET_DOOR_STATE_CLOSED, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)), NULL});
}
...
{
	return NEW_HOMEKIT_SERVICE(WINDOW_COVERING, .primary = true, .characteristics = (homekit_characteristic_t *[]){NEW_HOMEKIT_CHARACTERISTIC(NAME, szname), NEW_HOMEKIT_CHARACTERISTIC(CURRENT_POSITION, WINDOWCOVERING_POSITION_CLOSED, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)), NEW_HOMEKIT_CHARACTERISTIC(TARGET_POSITION, WINDOWCOVERING_POSITION_CLOSED, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)), NEW_HOMEKIT_CHARACTERISTIC(POSITION_STATE, WINDOWCOVERING_POSITION_STATE_STOPPED, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context)), NULL});
}
...
homekit_service_t *hap_new_switch_service(const char *szname, hap_callback cb, void *context)
{
	...
	return NEW_HOMEKIT_SERVICE(SWITCH, .primary = true, .characteristics = (homekit_characteristic_t *[]){NEW_HOMEKIT_CHARACTERISTIC(NAME, szname), NEW_HOMEKIT_CHARACTERISTIC(ON, true, .callback = HOMEKIT_CHARACTERISTIC_CALLBACK(cb, .context = context), ), NULL});
}
...
{
	if (!ch ||
		(ch->format != homekit_format_uint8 && ch->format != homekit_format_uint16 && ch->format != homekit_format_uint32 && ch->format != homekit_format_int))
	{
